# Тестирование PHP: PHPUnit, Codeception, Pest

Гайд для Lead/Senior PHP backend developer перед интервью. Фокус: как объяснять тестирование практически, какие trade-off'ы видеть, где чаще всего ломается стратегия тестов.
...
Частый Senior-комментарий: пирамида не догма. Для API/backend часто получается ромб или trophy: много интеграционных/API тестов, если ценность системы в связке БД, ORM, HTTP и очередей.
...
Риск: мок проверяет implementation details. Если тест знает слишком много о вызовах внутри сервиса, рефакторинг станет дорогим.
...
Вопрос: когда mock, а когда integration test?

Ответ: mock использую на границе с внешним side effect или для дорогой зависимости. Если риск в SQL/ORM/transaction behavior, нужен integration test с реальной БД.

Вопрос: почему 100% coverage недостаточно?

Ответ: coverage показывает выполнение строк, но не качество assert'ов. Код может быть покрыт и при этом не проверять важный результат. Для критичной логики полезен mutation testing.

Вопрос: как тестировать очередь?

Ответ: отдельно проверяю, что job dispatch'ится при нужном событии, и отдельно тестирую handler/job. Для критичных flows добавляю integration test с реальным или test transport.

Вопрос: как работать с flaky tests?

Ответ: сначала фиксирую причину и изоляцию: время, random, порядок, shared state, внешние сервисы. Quarantine допустим только временно, иначе команда перестает доверять CI.

Вопрос: что важнее, unit или integration?

Ответ: зависит от риска. Для чистой доменной логики unit дешевле. Для backend с ORM, транзакциями и очередями integration/API tests часто дают больше уверенности.

Вопрос: как тестировать legacy?

Ответ: пишу characterization tests вокруг текущего публичного поведения, затем рефакторю маленькими шагами и отделяю зависимости.
