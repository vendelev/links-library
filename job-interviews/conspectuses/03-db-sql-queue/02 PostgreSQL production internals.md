...
В этом файле WAL рассматривается как механизм durability и crash recovery.

Physical replication, logical replication, read replicas, replication lag,
read-after-write consistency, failover и split-brain разобраны в canonical-файле
...
- Понимаю WAL, checkpoints и crash recovery.
...
- Помню PHP-specific риски: N+1, long transactions, too many connections.
