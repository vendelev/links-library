...
Конспект о production-debugging на Linux: процессы, CPU, память, диск, сеть, логи, systemd, cron, permissions, strace, PHP-FPM, Docker и типовые инциденты.

## Ментальная модель инцидента

На интервью важно показать не набор команд, а способ быстро сузить область проблемы по фактам.
...
> Я начинаю не с перезапуска, а с определения bottleneck: CPU, память, диск, сеть, лимиты или приложение. Параллельно фиксирую timeline и ищу безопасный mitigation.
...
- `RES`: resident memory;
- `VIRT`: virtual memory, не равно реальному потреблению;
...
- zombie обычно симптом, а не причина; важнее родительский процесс.

Pitfall:

> Load average 20 на машине с 32 CPU может быть нормальным, а load 8 на машине с 2 CPU уже серьезный сигнал.
...
Если `mpstat` и `pidstat` не установлены, часто нужен пакет `sysstat`.
...
> High load не означает автоматически high CPU. Я проверяю runnable queue, blocked tasks, iowait и steal time. Если процессы в `D`, перезапуск приложения может не помочь, нужно смотреть storage.

## Память и OOM

### `free` и потребители памяти
...
> Строка `free` не означает, что память закончилась. Linux активно использует память под page cache.
...
- проверить PHP `memory_limit`, размер responses/batches и FPM `pm.max_children`.
...
- PHP-FPM pool/master наследуют лимиты от service manager.

Senior-ответ:

> Если приложение падает с `Too many open files`, я смотрю лимиты процесса через `/proc/<pid>/limits`, фактическое число fd через `/proc/<pid>/fd`, типы fd через `lsof`, а не только `ulimit` в своей shell-сессии.
...
- `CLOSE_WAIT`: remote закрыл соединение, local app не закрыла socket, часто bug/leak;
- `SYN_SENT`: не удается установить исходящее соединение;
- `SYN_RECV`: входящий handshake, возможна перегрузка или SYN flood.
...
> `systemctl restart` может временно скрыть проблему. Перед рестартом лучше собрать минимум фактов: status, последние логи, PID, лимиты, core/OOM признаки.

## Cron и periodic jobs
...
> Для периодических задач я проверяю overlap, lock, duration, exit code, environment и нагрузку на shared resources. Cron-инциденты часто выглядят как внезапный рост CPU/I/O в одно и то же время.
...
> `chmod 777` как решение на production неприемлем. Senior должен найти минимально нужные права и владельца.
...
- много `connect()` к Redis с ошибкой: connectivity или pool;
...
> Я не просто увеличиваю `pm.max_children`. Сначала считаю память на worker и ищу, почему workers заняты: CPU-bound PHP, ожидание БД, HTTP API, Redis, filesystem или deadlock. Иначе можно получить OOM вместо 502.
...
- Перезапустить сервис без сбора фактов: можно потерять evidence.
- High load значит CPU: нет, может быть I/O, lock, steal, uninterruptible sleep.
- Память закончилась, потому что `free` мало: page cache не равен утечке.
...
1. Найти топ-5 процессов по CPU и памяти через `ps`.
2. Сравнить `df -h` и `du -sh` для `/var/log`.
3. Запустить `curl -w` к HTTPS endpoint и разобрать DNS/connect/TLS/TTFB.
...
8. Включить PHP-FPM slowlog на staging и проверить запись медленного запроса.

## Self-check

1. Чем `RES` отличается от `VIRT` в `top`?
2. Почему load average может быть высоким при низком CPU?
3. Что означает много процессов в состоянии `D`?
4. Как найти удаленный файл, который все еще занимает место на диске?
5. Почему `free` memory в Linux не равна реально доступной памяти?
6. Где смотреть OOM killer events?
7. Почему `CLOSE_WAIT` опаснее, чем большое количество `TIME_WAIT`?
8. Как проверить, достиг ли PHP-FPM `pm.max_children`?
9. Почему нельзя бездумно увеличивать число PHP-FPM workers?
10. Чем лимит file descriptors в shell отличается от лимита systemd-сервиса?
11. Как разделить сетевую задержку на DNS, connect, TLS и TTFB?
12. Что проверить, если cron job работает вручную, но не работает из cron?
13. Почему после удаления лог-файла место может не освободиться?
14. Что проверить в Docker при внезапном рестарте контейнера?
15. Когда использовать `strace`, а когда лучше не трогать production-процесс?
