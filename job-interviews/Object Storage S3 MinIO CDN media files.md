...
Гайд для подготовки к интервью на позицию Lead/Senior PHP backend developer. Фокус: архитектура хранения файлов, безопасность, эксплуатация, стоимость, интеграция с PHP/Laravel/Symfony и типичные trade-off'ы.
...
Object Storage хорошо подходит для:
...
Object Storage плохо подходит для:

- частых мелких random-write операций;
- транзакционного обновления части файла;
- сценариев, где нужна POSIX-семантика lock/rename/fsync;
- хранения данных, которые должны часто изменяться in-place.
...
## 2. S3 как де-факто стандарт

Amazon S3 стал стандартом API для object storage. Многие системы совместимы с S3 API: MinIO, Ceph RGW, Wasabi, Backblaze B2 S3 API, DigitalOcean Spaces, Cloudflare R2.

Важные сущности S3:

- Bucket: контейнер для объектов.
- Key: имя объекта внутри bucket.
- Object: содержимое плюс метаданные.
...
Не стоит создавать bucket на каждого пользователя, если нет очень сильной причины. Обычно используют один или несколько bucket и разделяют объекты prefix'ами.

### Key

S3 key - это строка. Символ `/` не создает настоящие директории, а только формирует prefix.
...
- Закладывать tenant/user/project prefix для упрощения lifecycle и аудита.
...
Но metadata не заменяет БД. Для бизнес-сущностей лучше хранить canonical state в PostgreSQL/MySQL, а S3 использовать как blob storage.
...
## 4. Консистентность S3

Сейчас Amazon S3 предоставляет strong read-after-write consistency для `PUT`, `GET`, `LIST`, операций изменения tags, ACL и metadata.
...
- overwrite одного key может быть архитектурно опасен при CDN/cache;
- внешние системы вокруг S3 могут иметь собственную задержку: CDN, очереди, индексы, search, БД.

Senior-ответ:

> S3 сейчас strong consistent на уровне отдельных операций с объектами и LIST, но я все равно проектирую workflow так, чтобы состояние процесса фиксировалось в БД/очереди, а не выводилось только из наличия набора объектов.

## 5. Public и private files
...
> По умолчанию bucket должен быть private и иметь Block Public Access. Public-доступ я включаю осознанно либо через отдельный bucket/prefix, либо через CDN с контролируемой политикой.

## 6. Presigned URLs

Presigned URL - это URL с подписью, который временно разрешает выполнить конкретную операцию: `GET`, `PUT`, иногда multipart-related операции.
...
- URL является bearer token: кто получил ссылку, тот может использовать до истечения TTL;
- сложно отозвать отдельный presigned URL до истечения TTL;
- нужно валидировать MIME, размер, owner и expected key на backend до выдачи URL;
- после прямой загрузки нужен callback/polling/event, чтобы подтвердить факт загрузки и запустить обработку.

Типовой upload flow:

1. Клиент запрашивает у backend разрешение на загрузку.
...
- несовпадение lifecycle с бизнес-retention;
- восстановление из archive может занимать часы;
- lifecycle не мгновенный, не стоит использовать его как realtime scheduler.

Senior-ответ:

> Lifecycle я проектирую вместе с бизнес-retention и юридическими требованиями. Для временных upload'ов и incomplete multipart это must-have, для архивов надо учитывать restore time и реальные паттерны доступа.
...
- приложение должно понимать, работает ли оно с latest или конкретной version id.
...
- SSE-S3: default baseline для большинства данных.
- SSE-KMS: когда нужен контроль ключей, audit, rotation, separation of duties.
- Client-side: когда storage provider не должен видеть plaintext, но сложнее key management.
...
- CDN/origin access тоже должен учитывать KMS permissions.

Senior-ответ:

> Encryption at rest должен быть включен по умолчанию. Для чувствительных данных я выбираю KMS с отдельными ключами и аудитом, но заранее считаю стоимость и лимиты KMS на high-throughput workloads.

## 11. Access policies и безопасность
...
Лучше ограничивать конкретными действиями и ресурсами:
...
Для immutable ассетов с content hash в имени можно ставить долгий TTL.

Для mutable key лучше короткий TTL или versioned URL.
...
Если файл перезаписывается, CDN может долго отдавать старую версию.
...
- доступ к одному файлу;
- короткоживущая ссылка;
- download link.

Когда Signed Cookies:
...
Типовая pipeline для пользовательского медиа:
...
В PHP-проектах heavy media processing часто выносят в workers:
...
- в key удобно включать размер/формат/версию алгоритма;
- при изменении алгоритма обработки использовать новый prefix/version;
...
> Я версионирую не только original, но и pipeline derivatives. Если меняется алгоритм ресайза или формат, новый `vN` в key позволяет избежать конфликтов с CDN cache.

## 16. Virus scanning

Для пользовательских файлов virus scanning часто обязателен.
...
Для tus/resumable uploads можно использовать отдельный upload service или библиотеку, но все равно финальное хранилище часто остается S3-compatible.
...
- поддержка erasure coding, versioning, policies, replication.
...
Для local dev часто используют endpoint:

```text
http://minio:9000
```

И path-style addressing:
...
## 19. Backups и disaster recovery
...
## 23. Senior answers на интервью

### Как организовать загрузку больших файлов?

> Я бы не проксировал большие файлы через PHP. Backend выдает presigned multipart upload после проверки прав и лимитов, хранит upload state в БД, клиент грузит parts напрямую в S3, затем backend подтверждает объект через `HEAD`, проверяет size/checksum и запускает async scan/processing. Incomplete multipart чистится lifecycle policy.

### Как сделать приватные файлы через CDN?

> Bucket остается private, CloudFront получает доступ к origin через OAC, пользователю выдаются signed URL или signed cookies. Для видео с HLS лучше signed cookies, потому что сегментов много. Origin напрямую из интернета недоступен.

### Как избежать проблем с CDN cache при обновлении аватара?

> Не перезаписывать `avatar.jpg`, а писать новый immutable key с версией/ULID/hash, например `avatar/01J...webp`, обновлять ссылку в БД и отдавать долгий `Cache-Control`. Invalidation использовать только как fallback.

### Где хранить metadata файла?

> Технические HTTP metadata можно хранить в object metadata, но бизнес-состояние, owner, статус scan/processing, размер, checksum, связи с сущностями - в БД. S3 не заменяет транзакционную модель.
...
## 24. Архитектурный шаблон для SaaS media storage
...
- Чем object storage отличается от file/block storage?
- Почему key с `/` не является настоящей директорией?
- Какие данные стоит хранить в БД, а какие в object metadata?
- Что означает strong consistency в S3 и чего она не гарантирует?
- Почему presigned URL не заменяет авторизацию?
- Когда нужен multipart upload?
- Почему incomplete multipart uploads опасны?
- Как защитить private bucket за CDN?
- Когда использовать signed URL, а когда signed cookies?
- Почему immutable keys лучше invalidation?
- Как построить pipeline virus scan и thumbnails?
- Почему нельзя доверять `Content-Type` от клиента?
- Чем durability отличается от backup?
- Какие cost drivers есть у S3/CDN?
- Какие особенности MinIO важны в production?

## 26. Mini-practice

### Задание 1

Спроектируй upload flow для видео 2-5 GiB в Laravel API.
...
Пользователь обновил аватар, но CDN продолжает отдавать старый.
